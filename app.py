...
def load_link_of_qs():
    with open("Leetcode_Scrapper/Qdata/Qindex.txt", "r") as f:
        links = f.readlines()
...
def calc_docs_sorted_order(q_terms):
    ...
    for term in q_terms:
        ...
        for doc in tf_vals_by_docs:
            if doc not in potential_docs:
                ...
                sumidf_doc[doc] = get_idf_value(term)
            else:
                potential_docs[doc] += tf_vals_by_docs[doc]*idf_value
                sumidf_doc[doc] += get_idf_value(term)
    ...
    for doc in potential_docs:
        ...
        potential_docs[doc] = potential_docs[doc]*sumidf_doc[doc]/sumidf
    ...
    if (len(potential_docs) == 0):
        ans.append({"Question Link":"No matching question found. Please search with more relevant terms.","Score":"0"})
    ...
    for doc_index in potential_docs:
        ...
        ans.append({"Question Link": Qlink[int(doc_index) - 1], "Score": potential_docs[doc_index]})
    return ans
